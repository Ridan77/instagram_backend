...
import { toggleFollow } from './user.controller.js'
...
async function addLikeUser(loggedinUser, storyId) {
    try {
        const collection = await dbService.getCollection(collectionName)
        ...
        let update
        if (user) {
            update = { $pull: { likedStoryIds: storyId } }
        } else {
...
async function addOrRemoveFollow(loggedinUser, userToFollowId) {
    try {
        ...
        if (user) {
            updateMe = { $pull: { following: { _id: other._id } } }
            updateOther = { $pull: { followers: { _id: loggedinUser._id } } }
        } else {
            updateMe = { $addToSet: { following: otherMiniUser } }
